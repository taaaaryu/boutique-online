...
GENERATION = 10
NUM_NEXT = 10
all_deployments = ["adservice", "cartservice", "checkoutservice", "currencyservice", "emailservice", "paymentservice","frontend","productcatalogservice", "recommendationservice", "shippingservice"]
NAMESPACE = "boutique"
...
algo_interval = 300
kill_interval = 40
pause_interval = 0.5*kill_interval
log_interval = 20
...
def kill_sidecar_timer(spec, logger, **kwargs):
    ...
    logging.info("start killing sidecars")
    ...
    for service_idx, deployment in enumerate(all_deployments):
        ...
        if not service_pods:
            continue
        logging.info(f"Found {len(service_pods)} pods for deployment {deployment}")

        group_id = get_group_id(service_idx)
        if group_id == -1 or group_id in already_killed_groups:
            continue

        victim = random.choice(service_pods)
        threading.Thread(target=pause_pod, args=(v1, victim.metadata.name, NAMESPACE, logger, deployment)).start()
        logger.info(f"Triggered pause for pod: {victim.metadata.name} from deployment {deployment}")
        ...
        for other_service_idx, val in enumerate(service_groups[group_id]):
            ...
            logging.info(f"Found {len(sibling_pods)} sibling pods for deployment {dep_name}")
            if sibling_pods:
                sib_victim = random.choice(sibling_pods)
                threading.Thread(target=pause_pod, args=(v1, sib_victim.metadata.name, NAMESPACE, logger, dep_name)).start()
    csv_filename = "pod_status.csv"
    df = pd.read_csv(csv_filename)
    df.loc[df.index[-1], "pause_flag"] = 1
...
def pause_pod(v1, pod_name, namespace, logger, deployment, duration=pause_interval, container_name='server'):
    global pause_counts, paused_pods
    try:
        pid_command = ['sh', '-c', 'ps -eo pid,comm | grep java | grep -v grep | awk \'{print $1}\'']
        pid = stream(
        ...
        stop_command = ['kill', '-STOP', pid.strip()]
        stream(
            v1.connect_get_namespaced_pod_exec,
            pod_name, namespace,
        ...
        # ここでresume予定時刻も記録する
        paused_pods[pod_name] = time.time() + duration

        pause_counts[deployment] += 1
        print(f"[Thread] Paused pod {pod_name} for {duration} seconds")

        time.sleep(duration)

        cont_command = ['kill', '-CONT', pid.strip()]
        stream(
            v1.connect_get_namespaced_pod_exec,
            pod_name, namespace,
        ...
        print(f"[Thread] Resumed pod {pod_name}")
        # resumeが終わったらpaused_podsから消してもいいかも（やるならここで）
        # paused_pods.pop(pod_name, None)

    except Exception as e:
        print(f"[Thread] Failed to pause/resume pod {pod_name}: {e}")
